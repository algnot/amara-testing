...
Cypress.Commands.add("login", (email: string, password: string) => {
  cy.session([email, password], () => {
    ...
    cy.get('input[type="password"], input[name="password"]')
      ...
      .type(password);
    cy.get(
      'button[type="submit"], button:contains("Log in"), [data-cy="login"]',
    )
      .first()
      .click();
...
Cypress.Commands.add("createStudent", () => {
  ...
  getAndType('input[placeholder="นามสกุล (ภาษาไทย)"]', studentData.lastNameTH);
  getAndType('input[placeholder="ชื่อ (อังกฤษ)"]', studentData.firstNameEN);
  getAndType('input[placeholder="นามสกุล (อังกฤษ)"]', studentData.lastNameEN);
