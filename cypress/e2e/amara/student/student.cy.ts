...
import {
  getAndExpectContain,
  getAndType,
  getInputByLabelAndExpect,
} from "@/cypress/util/helper";
...
describe("test student", () => {
  it("should error with no sale person", () => {
    ...
    getAndType('input[placeholder="ชื่อ (ภาษาไทย)"]', studentData.firstNameTH);
    getAndType(
      'input[placeholder="นามสกุล (ภาษาไทย)"]',
      studentData.lastNameTH,
    );
    getAndType(
      'input[placeholder="ชื่อ (อังกฤษ)"]',
      studentData.firstNameEN,
    );
    getAndType(
      'input[placeholder="นามสกุล (อังกฤษ)"]',
      studentData.lastNameEN,
    );
    getAndType(
      'input[placeholder="รหัส CS"]',
      "auto888auto888auto888auto888auto888auto888",
    );

    cy.get('button[type="submit"]').click();
    getAndExpectContain('[role="alertdialog"]', [
      "ผิดพลาด",
      "ไม่พบรหัส CS นี้ในระบบ",
    ]);
  });
  ...
  it("should error with duplicate student", async () => {
    ...
    getAndType('input[placeholder="ชื่อ (ภาษาไทย)"]', "ศิริวรรณ");
    getAndType('input[placeholder="นามสกุล (ภาษาไทย)"]', "หนอกค้างพลู");
    getAndType('input[placeholder="ชื่อ (อังกฤษ)"]', "Sirivan");
    getAndType('input[placeholder="นามสกุล (ภาษาอังกฤษ)"]', "Nuokkhangplu");
    getAndType('input[placeholder="รหัส CS"]', "auto888");
    cy.get('button[type="submit"]').click();

    getAndExpectContain('[role="alertdialog"]', [
      "ผิดพลาด",
      "ข้อมูลนักเรียน ศิริวรรณ หนอกค้างพลู (Sirivan Nuokkhangplu) มีอยู่แล้วในระบบ รหัสนักเรียน 250400119 ไม่สามารถสร้างซ้ำได้",
    ]);
  });
  ...
  it("should find student in dashboard using stored data", () => {
    ...
    cy.contains("td", student.id).should("be.visible").parent("tr").click();
