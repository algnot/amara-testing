...
describe("test draft certificate with auth", () => {
  ...
  it("should error with not found student", () => {
    ...
    cy.get('button[type="submit"]').click();
    getAndExpectContain('[role="alertdialog"]', [
      "ไม่พบข้อมูลนักเรียนดังกล่าวในระบบ",
    ]);
  });
  ...
  it("should error with no start date and end date", () => {
    ...
    getAndExpectContain('[role="alertdialog"]', [
      "กรุณาใส่ข้อมูลต่อไปนี้ start_date, end_date",
    ]);
  });
  ...
  it("should create draft certificate", () => {
    ...
    cy.get('input[placeholder="รหัสนักเรียน"]').should(
      "have.value",
      student.id,
    );
    cy.get('input[placeholder="ชื่อนักเรียน (ไทย)"]').should(
      "have.value",
      `${student.firstNameTH} ${student.lastNameTH}`,
    );
    cy.get('input[placeholder="ชื่อนักเรียน (อังกฤษ)"]').should(
      "have.value",
      `${student.firstNameEN} ${student.lastNameEN}`,
    );

    cy.contains("button", "เลือก").click();
    cy.get('div[role="menuitem"]:visible')
    ...
    getAndType(
      'input[placeholder="วันที่เริ่มเรียน"]',
      getDateInputFormat(today),
    );

    cy.get('input[placeholder="วันที่เรียนจบ"]').click();
    getAndType(
      'input[placeholder="วันที่เรียนจบ"]',
      getDateInputFormat(tomorrow),
    );

    cy.contains("button", "ขอใบประกาศ").click();
    getAndExpectContain('[role="alertdialog"]', [
      "ส่งคำขอสำเร็จ",
      "ระบบได้ส่งคำขอสร้างใบประกาศเรียบร้อยแล้ว",
    ]);
  });
});
